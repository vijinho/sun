...
function sunCoords($d)
{
    ...
    return new DecRa(
        declination($L, 0), rightAscension($L, 0)
    );
}
...
{ // geocentric ecliptic coordinates of the moon
    ...
    return new DecRaDist(
        declination($l, $b), rightAscension($l, $b), $dt
    );
}
...
class SunCalc
{
    ...
    public function getSunPosition()
    {
        ...
        return new AzAlt(
            azimuth($H, $phi, $c->dec), altitude($H, $phi, $c->dec)
        );
    }
    ...
    public function getMoonIllumination()
    {
        ...
        $angle = atan2(cos($s->dec) * sin($s->ra - $m->ra),
            sin($s->dec) * cos($m->dec) - cos($s->dec) * sin($m->dec) * cos($s->ra
                - $m->ra));
    ...
    public function getMoonTimes($inUTC = false)
    {
        ...
        for ($i = 1; $i <= 24; $i += 2) {
            $h1 = $this->getMoonPosition(hoursLater($t, $i), $this->lat,
                    $this->lng)->altitude - $hc;
            $h2 = $this->getMoonPosition(hoursLater($t, $i + 1), $this->lat,
                    $this->lng)->altitude - $hc;
